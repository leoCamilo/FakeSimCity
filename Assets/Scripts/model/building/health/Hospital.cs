            ...
            if (IsBuildable(cell))
            {
                ...
                for (int i = start_line; i <= end_line; i++)
                    for (int j = start_column; j <= end_column; j++) {
                        ...
                        _mat[i, j].status[(int)Status.Health] += _influenceBound / (new Vector3(tmp.x, 0, tmp.y) -  pos).magnitude;
                    }

                // cell.obj.GetComponent<AudioSource>().Play();
            }
        ...
        public static void RemoveFromMap(Vector3 pos)
        {
            ...
            for (int i = start_line; i <= end_line; i++)
                for (int j = start_column; j <= end_column; j++)
                {
                    ...
                    mat[i, j].status[(int)Status.Health] -= _influenceBound / (new Vector3(tmp.x, 0, tmp.y) - pos).magnitude;
                }
        }
    }
}
